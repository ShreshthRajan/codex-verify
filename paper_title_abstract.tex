% Title and Abstract for CodeX-Verify Paper
% Copy this into your Overleaf document

\title{CodeX-Verify: Multi-Agent Verification of LLM-Generated Code via Compound Vulnerability Detection and Information-Theoretic Ensemble}

% Authors - UPDATE WITH YOUR INFO
\author{
...
% Keywords for ArXiv
\keywords{Multi-agent systems, Code verification, LLM-generated code, Static analysis, Compound vulnerabilities, Information theory, Ensemble learning}

% ACM CCS Concepts (if needed for ICML/ICSE)
\begin{CCSXML}
<ccs2012>
<concept>
...
The term $P(\text{chain succeeds})$ represents the attacker's ability to combine vulnerabilities, creating attack vectors unavailable to either vulnerability alone. For example, SQL injection ($v_1$) alone provides limited query manipulation; hardcoded credentials ($v_2$) alone are unexploitable without injection capabilities. Their combination enables full database access, a capability unavailable to either vulnerability individually.

Normalizing to risk scores, $P(\text{chain succeeds}) = \alpha(v_1, v_2)$ where $\alpha > 1$ quantifies the multiplicative advantage. In our formulation:
\begin{equation}
    \text{Risk}_{\text{compound}} = \text{Risk}(v_i) \times \text{Risk}(v_j) \times \alpha(v_i, v_j)
\end{equation}
\end{proof}

\textbf{Comparison to Additive Models.} Traditional risk assessment computes total risk as $R_{\text{linear}} = \sum_i \text{Risk}(v_i)$. For SQL injection (risk = 10) and hardcoded credentials (risk = 10), linear models yield total risk = 20. Our exponential model with $\alpha = 3.0$ yields:
\begin{equation}
    R_{\text{compound}} = 10 + 10 + (10 \times 10 \times 3.0 - 10 - 10) = 300
\end{equation}

This 15$\times$ amplification reflects documented real-world impact: isolated SQL injection enables limited reconnaissance, isolated credentials are unexploitable, but their combination enables complete database compromise with persistent access~\cite{sheyner2002attack, owasp2024}.

\textbf{Amplification Factor Calibration.} We empirically set $\alpha(v_i, v_j)$ based on attack chain literature:
\begin{align}
    \alpha(\text{SQL injection}, \text{hardcoded secret}) &= 3.0 \quad \text{(full DB access)} \nonumber \\
    \alpha(\text{code execution}, \text{dangerous import}) &= 2.0 \quad \text{(system compromise)} \nonumber \\
    \alpha(\text{complexity}, \text{algorithm inefficiency}) &= 1.8 \quad \text{(cascading failure)}
\end{align}

Higher $\alpha$ values reflect stronger synergistic exploitation potential, calibrated from CVSS impact scores and attack graph analysis~\cite{cvss2024}.
...
\midrule
Multi-agent $>$ single-agent & +39.7pp improvement & \checkmark \\
Diminishing returns with more agents & +14.9pp, +13.5pp, +11.2pp & \checkmark \\
Agent correlation $\rho \approx 0$ (orthogonal) & Measured $\rho = 0.05$--$0.25$ & \checkmark \\
Sample $n=99$ gives $\pm$9\% CI & Measured $\pm$9.1\% CI & \checkmark \\
Accuracy $\geq 51.7\%$ (PAC bound) & Measured 68.7\% & \checkmark \\
Optimal $n^* = 4$ agents & Marginal gains <10pp for agent 5 & \checkmark \\
Compound $\alpha > 1$ improves detection & Compound detection active & \checkmark \\
\bottomrule
\end{tabular}
\end{table}
        ...
        (AST, edge  (Vuln    (Algo  (Quality
         cases)     patterns) complex) metrics)
           ...
           compound vulnerability detection)
            ...
            (FAIL / WARNING / PASS)
\end{verbatim}
\caption{CodeX-Verify architecture. Four specialized agents analyze code in parallel, with results aggregated via weighted combination and compound vulnerability detection before final deployment decision.}
\label{fig:architecture}
\end{figure}

\textbf{Design Principles.} Our architecture follows three key principles: (1) \emph{Agent specialization}: Each agent focuses on orthogonal bug dimensions (validated by $\rho = 0.05$--$0.25$ correlation), enabling complementary detection as predicted by Theorem~\ref{thm:information}. (2) \emph{Parallel execution}: Agents run asynchronously via Python's \texttt{asyncio}, achieving sub-200ms total latency through concurrent analysis. (3) \emph{Hierarchical aggregation}: Individual agent verdicts combine via weighted scores, then undergo compound vulnerability analysis, finally producing a multi-threshold deployment decision respecting asymmetric security costs.
...
\Procedure{DetectCompounds}{$V$}
    \State $C \gets \emptyset$ \Comment{Compound vulnerabilities}
    \For{$(v_i, v_j) \in V \times V$ where $i < j$}
        \If{$(v_i.\text{type}, v_j.\text{type}) \in E$} \Comment{Known attack chain}
...
\textbf{Caching Layer.} Multi-tier caching (in-memory LRU + disk-based) with TTL-based invalidation. Cache key: $\texttt{SHA256}(\text{code} || \text{config} || \text{agent\_versions})$. Typical hit rate: 60--70\% in CI/CD scenarios with repeated file analysis. Cache disabled for our evaluation to ensure independent measurements.

\textbf{Extensibility.} Agent interface (\texttt{BaseAgent} abstract class) enables custom agents via:
\begin{verbatim}
class CustomAgent(BaseAgent):
    async def _analyze_implementation(self, code, context):
        # Domain-specific analysis
        issues = detect_custom_patterns(code)
        return AgentResult(issues=issues, score=calculate_score(issues))
\end{verbatim}

Configuration via YAML allows enabling/disabling agents, adjusting thresholds, and setting weights without code changes, supporting deployment-specific tuning (strict enterprise vs. permissive development environments).
...
\textbf{AST-Based Metrics.} Traverses Python AST computing structural metrics:
\begin{itemize}[leftmargin=*, itemsep=0pt]
    \item Cyclomatic complexity: $M = 1 + \#\{\texttt{if}, \texttt{while}, \texttt{for}, \texttt{except}\}$
    \item Nesting depth: Maximum depth of nested control structures
    \item Exception coverage: $\#\{\text{functions with } \texttt{try}\} / \#\{\text{functions with risky ops}\}$
    \item Input validation: $\#\{\text{functions with validation}\} / \#\{\text{functions with params}\}$
\end{itemize}

\textbf{Exception Handling Detection.} Identifies functions calling \texttt{open()}, \texttt{request()}, \texttt{json.loads()}, or \texttt{pickle.loads()} without enclosing \texttt{try-except}. Flags bare \texttt{except} clauses (catch-all error hiding) and empty exception handlers (\texttt{except: pass}) as production anti-patterns.

\textbf{Edge Case Patterns.} Searches function bodies for:
\begin{itemize}[leftmargin=*, itemsep=0pt]
    \item \texttt{None} checks: \texttt{if x is None}
    \item Empty collection checks: \texttt{if not lst} or \texttt{if len(lst) == 0}
    \item Boundary validation: \texttt{if 0 <= index < len(arr)}
\end{itemize}

Functions accepting inputs without these checks flagged for edge case risk. Coverage score: (\# functions with edge case handling) / (\# functions with inputs).

\textbf{Contract Validation.} Parses docstrings (when present) for promises:
\begin{itemize}[leftmargin=*, itemsep=0pt]
    \item If docstring mentions ``raises,'' validates implementation contains \texttt{raise} statements
    \item If docstring mentions ``returns,'' validates function has non-None \texttt{return}
    \item Flags contract violations as MEDIUM severity (promises not fulfilled)
\end{itemize}
...
\Procedure{DecideDeployment}{$S_{\text{sys}}$, $\mathcal{I}$}
    \State Partition $\mathcal{I}$ by severity and agent source
    \If{$|\mathcal{I}_{\text{CRITICAL}}| > 0$ or compound vulnerabilities detected}
        \State \Return FAIL \Comment{Zero tolerance}
    \ElsIf{$|\mathcal{I}_{\text{HIGH}}^{\text{security}}| \geq 1$}
        \State \Return FAIL \Comment{Any security HIGH blocks}
    \ElsIf{$|\mathcal{I}_{\text{HIGH}}^{\text{correctness}}| \geq 2$}
        \State \Return FAIL \Comment{Multiple correctness issues block}
    \ElsIf{$|\mathcal{I}_{\text{HIGH}}^{\text{correctness}}| = 1$ or $S_{\text{sys}} \in [0.50, 0.85]$}
        \State \Return WARNING \Comment{Human review recommended}
    \ElsIf{$S_{\text{sys}} \geq 0.70$ and no high security/correctness issues}
        \State \Return PASS
    \Else
        \State \Return WARNING \Comment{Low score but no critical bugs}
    \EndIf
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\textbf{Reproducibility Package.} We release: (1) Complete source code (6,122 lines across 4 agents, orchestrator, aggregator)~\footnote{Code available at: \url{https://github.com/your-org/codex-verify}}, (2) 99-sample benchmark with ground truth labels (JSON format), (3) Evaluation scripts reproducing all results, (4) Docker container for environment isolation, (5) Ablation study results (15 configurations $\times$ 99 samples). All random seeds fixed (\texttt{random\_state=42}) for deterministic reproduction.
...
\textbf{Diminishing Returns Confirmed.} Marginal gains from adding agents 2, 3, 4 are +14.9pp, +13.5pp, +11.2pp respectively (Figure~\ref{fig:diminishing}), confirming Theorem~\ref{thm:diminishing}'s prediction of sublinear information gain. Extrapolating the trend $(14.9, 13.5, 11.2) \to \approx 9.0$ suggests agent 5 would contribute <10pp, validating our claim that $n^* = 4$ is near-optimal (Corollary 1).
...
FPR
 |
60% +  Codex (40% TPR, 60% FPR)
 |     [No verification baseline]
 |
50% +  CodeX-Verify (76% TPR, 50% FPR) <-- OURS
 |     [Static analysis, multi-agent]
 |
35% +  Static Analyzers (65% TPR, 35% FPR)
 |     [Traditional SAST tools]
 |
 |
10% +  Meta Prompt (75% TPR, 8.6% FPR)
 |     [Test-based, different methodology]
 |
  +----+----+----+----+----+----+----+----+-> TPR
  0%  40%  50%  65%  75%  80%  90% 100%

Ideal region (top-right): High TPR, Low FPR
Our position: Competitive TPR (76%, matching Meta),
              Higher FPR (50% vs. 8.6%, static vs. test tradeoff)
\end{verbatim}
\caption{TPR-FPR comparison. Our system (76\% TPR, 50\% FPR) achieves competitive bug detection matching Meta Prompt's 75\% TPR while operating via static analysis. The FPR gap (50\% vs. 8.6\%) reflects methodology differences: static analysis flags quality issues (missing exception handling, edge cases) that test-based methods miss.}
...
Predicts agent 5: <10pp gain (not cost-effective)
\end{verbatim}
\caption{Multi-agent scaling: Progressive accuracy improvement with diminishing marginal returns. Adding agents 2, 3, 4 yields +14.9pp, +13.5pp, +11.2pp respectively, validating Theorem~\ref{thm:diminishing}'s prediction of sublinear information gain.}
...
We thank the reviewers for their constructive feedback. This work was supported by [YOUR FUNDING SOURCE]. Code and data available at \url{https://github.com/your-org/codex-verify}.
...
\midrule
SQL injection (direct) & \texttt{execute(...\textbackslash\%...)} & CWE-89 & HIGH \\
SQL injection (format) & \texttt{SELECT.*\.format\(} & CWE-89 & HIGH \\
SQL injection (f-string) & \texttt{f"SELECT.*\{.*\}"} & CWE-89 & HIGH \\
Command injection (concat) & \texttt{os.system.*\textbackslash+} & CWE-78 & HIGH \\
Shell injection & \texttt{shell\textbackslash s*=\textbackslash s*True} & CWE-78 & MEDIUM \\
\texttt{eval()} execution & \texttt{eval\textbackslash s*\textbackslash(} & CWE-94 & CRITICAL \\
\texttt{exec()} execution & \texttt{exec\textbackslash s*\textbackslash(} & CWE-94 & CRITICAL \\
\texttt{pickle.loads} unsafe & \texttt{pickle\.loads\textbackslash s*\textbackslash(} & CWE-502 & HIGH \\
\texttt{yaml.load} unsafe & \texttt{yaml\.load\textbackslash s*\textbackslash(} & CWE-502 & HIGH \\
MD5 usage & \texttt{hashlib\.md5\textbackslash s*\textbackslash(} & CWE-327 & MEDIUM \\
SHA1 usage & \texttt{hashlib\.sha1\textbackslash s*\textbackslash(} & CWE-327 & MEDIUM \\
Weak random & \texttt{random\.(randint|choice)} & CWE-338 & MEDIUM \\
Hardcoded password & \texttt{password\textbackslash s*=\textbackslash s*['"]} & CWE-798 & HIGH \\
Hardcoded API key & \texttt{api\_key\textbackslash s*=\textbackslash s*['"]} & CWE-798 & HIGH \\
Timing attack & \texttt{password|token.*==} & CWE-208 & MEDIUM \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{algorithmic}[1]
\Procedure{ClassifyComplexity}{$\text{code}$}
    \State $\text{tree} \gets \texttt{ast.parse}(\text{code})$
    \State $\text{max\_nest} \gets \textsc{MaxNestedLoops}(\text{tree})$
    \State $\text{has\_recursion} \gets \textsc{DetectRecursion}(\text{tree})$
    \If{$\text{has\_recursion}$}
        \If{$\textsc{IsTailRecursive}(\text{tree})$}
            \State \Return $O(n)$
        \Else
            \State \Return $O(2^n)$
        \EndIf
    \ElsIf{$\text{max\_nest} = 0$}
        \State \Return $O(1)$
    \ElsIf{$\text{max\_nest} = 1$}
        \State \Return $O(n)$ or $O(n \log n)$ \Comment{Check for efficient ops}
    \ElsIf{$\text{max\_nest} = 2$}
        \State \Return $O(n^2)$
    \Else
        \State \Return $O(n^{\text{max\_nest}})$
    \EndIf
\EndProcedure
\end{algorithmic}
...
\begin{itemize}[leftmargin=*, itemsep=2pt]
    \item[\checkmark] \textbf{Code:} Complete implementation (6,122 lines) at \url{https://github.com/your-org/codex-verify}
    \item[\checkmark] \textbf{Data:} 99-sample benchmark with ground truth labels (JSON format)
    \item[\checkmark] \textbf{Environment:} Dockerfile with exact dependencies (Python 3.10, all packages pinned)
    \item[\checkmark] \textbf{Seeds:} All random operations use \texttt{random\_state=42}
    \item[\checkmark] \textbf{Hyperparameters:} All thresholds, weights listed in Section~5.6 and Appendix A
    \item[\checkmark] \textbf{Evaluation scripts:} Automated reproduction of all results (\texttt{swe\_bench\_mirror\_evaluator.py}, \texttt{ablation\_study.py})
    \item[\checkmark] \textbf{Compute requirements:} Consumer hardware (16GB RAM, <3 hours), no specialized infrastructure
    \item[\checkmark] \textbf{Results:} All raw results (JSON) and processed metrics (CSV) included
\end{itemize}
...
\Procedure{HybridVerify}{code, prompt}
    \State $\text{static\_result} \gets \textsc{CodeX-Verify}(\text{code})$
    \If{$\text{static\_result} = \text{FAIL}$}
        \State \Return FAIL \Comment{Immediate rejection}
    \ElsIf{$\text{static\_result} = \text{PASS}$}
        \State $\text{paraphrases} \gets \textsc{GenerateParaphrases}(\text{prompt}, k=5)$
        \State $\text{variants} \gets [\textsc{LLM}(p) \text{ for } p \text{ in paraphrases}]$
        \State $\text{outputs} \gets [\textsc{Execute}(v, \text{test\_inputs}) \text{ for } v \text{ in variants}]$
        \If{$\textsc{OutputsConsistent}(\text{outputs})$}
            \State \Return PASS \Comment{Static + dynamic both pass}
        \Else
            \State \Return FAIL \Comment{Inconsistent outputs}
        \EndIf
    \Else
        \State \Return WARNING \Comment{Static uncertain, skip expensive dynamic}
    \EndIf
\EndProcedure
\end{algorithmic}
